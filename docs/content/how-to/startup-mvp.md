...
Zero-infra to production-grade secrets in three steps. No secrets in env vars, no plaintext in files — that's the whole point (see [why we don't trust environment variables]({{< relref "/introduction" >}})).

This guide strings together the [1Password]({{< relref "onepassword" >}}) and [AWS Secrets Manager]({{< relref "aws" >}}) how-tos into a growth path: start with zero infrastructure, migrate when the team and cluster arrive — the application code never changes, only one line of config.
